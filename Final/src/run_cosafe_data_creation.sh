...
#SBATCH --output=llama3_cosafe_data_creation_%j.out
...
OUTPUT_DIR="./results/cosafe_data"
mkdir -p $OUTPUT_DIR

echo "Starting CoSAFE-like data generation with Llama 3 at $(date)"

# Run the Python script
python generate_data.py \
    --input_file ~/CoSafe-Dataset/CoSafe datasets/self_harm.json \
    --output_file $OUTPUT_DIR/llama3_data.jsonl \
    --model_name meta-llama/Meta-Llama-3-8B-Instruct \
    --max_new_tokens 512 \
    --temperature 0.7 \
    --top_p 0.9 \
    --batch_size 1 \
    --num_samples 100  # Remove or adjust this parameter if you want to process the entire dataset
